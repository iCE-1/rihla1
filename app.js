...
grant select, insert, update, delete on srs_units to authenticated;</pre>
    <ol start="4">
...
function srsSchedule(kind, customDays, reviewCount){
  const total = srsTotalDays(kind, customDays);
  const n = Math.max(1, reviewCount || 7);
  const days = [];
  for(let i=1;i<=n;i++){
    const ratio = Math.pow(i/n, 1.6);
    days.push(Math.max(1, Math.round(ratio * total)));
  }
  return [...new Set(days)].sort((a,b)=>a-b);
}

function srsNextReviewInfo(unit){
  const schedule = srsSchedule(unit.duration_kind, unit.duration_days, unit.review_count);
  const stage = unit.current_stage || 0;
  if(stage >= schedule.length){
    return {done:true, total:schedule.length};
  }
  const startDate = new Date(unit.start_date);
  const nextDate = new Date(startDate);
  nextDate.setDate(nextDate.getDate() + schedule[stage]);
  const today = new Date();
  today.setHours(0,0,0,0);
  nextDate.setHours(0,0,0,0);
  const diffDays = Math.round((nextDate - today) / 86400000);
  return {done:false, nextDate, stageNum:stage+1, total:schedule.length, diffDays};
}
    ...
    <div class="srs-schedule-list">
      ${full.map(item => `
        <div class="srs-schedule-row ${item.completed ? 'srs-done' : ''}">
          <span>${item.completed ? '✓' : item.num}. ${formatDate(item.dateStr)}</span>
...
function openSrsForm(existing){
  editingSrsId = existing ? existing.id : null;
  const wrap = document.getElementById('srsFormWrap');
  const u = existing || {title:'', start_date: toDateStr(new Date()), duration_kind:'3m', duration_days:90, current_stage:0, review_count:7, tags:''};
  const kind = u.duration_kind || '3m';
  const reviewCount = u.review_count || 7;
  const schedule = srsSchedule(kind, u.duration_days, reviewCount);
  wrap.innerHTML = `
    <div class="entry-box">
      <div class="field">
      ...
      <div class="row2">
        <div class="field">
          ...
          <input type="date" id="srsStartDate" value="${u.start_date}">
        </div>
        <div class="field">
          <label>مدة التكرار</label>
          <select id="srsDurationKind" onchange="changeSrsDurationKind(this.value)">
            ${Object.entries(SRS_DURATION_LABELS).map(([k,v])=>`<option value="${k}" ${kind===k?'selected':''}>${v}</option>`).join('')}
          </select>
        </div>
      </div>
      <div id="srsCustomDaysWrap"></div>
      <div class="field" style="max-width:220px">
        <label>عدد المراجعات خلال هذي المدة</label>
        <input id="srsReviewCount" type="text" inputmode="numeric" value="${reviewCount}" onchange="changeSrsReviewCount(this.value)">
      </div>
      <div class="field">
        <label>وين وصلت؟ (لو عندك وحدة قديمة من مكان ثاني)</label>
        <select id="srsCurrentStage">
          <option value="0" ${(u.current_stage||0)===0?'selected':''}>لسا ما بدأت أي مراجعة</option>
          ${schedule.map((d,i)=>`<option value="${i+1}" ${(u.current_stage||0)===i+1?'selected':''}>خلصت المراجعة رقم ${i+1} (من ${schedule.length})</option>`).join('')}
        </select>
      </div>
      <div class="field">
  ...
  `;
  renderSrsCustomDaysField(kind, u.duration_days);
  wrap.scrollIntoView({behavior:'smooth', block:'start'});
}

function renderSrsCustomDaysField(kind, days){
  const elx = document.getElementById('srsCustomDaysWrap');
  if(!elx) return;
  if(kind === 'custom'){
    elx.innerHTML = `
      <div class="field" style="max-width:220px">
        <label>عدد الأيام الكلي</label>
        <input id="srsCustomDays" type="text" inputmode="numeric" value="${days && days>0 ? days : 90}" onchange="changeSrsCustomDays(this.value)">
      </div>
    `;
  } else {
    elx.innerHTML = '';
...
function changeSrsDurationKind(newKind){
  const draft = {
    id: editingSrsId,
    title: document.getElementById('srsTitle').value,
    start_date: document.getElementById('srsStartDate').value,
    duration_kind: newKind,
    duration_days: newKind==='custom' ? 90 : srsTotalDays(newKind),
    current_stage: 0,
    review_count: parseInt(normalizeDigits(document.getElementById('srsReviewCount').value)) || 7,
    tags: document.getElementById('srsTags').value
  };
  openSrsForm(draft);
}

function changeSrsCustomDays(val){
  const draft = {
    id: editingSrsId,
    title: document.getElementById('srsTitle').value,
    start_date: document.getElementById('srsStartDate').value,
    duration_kind: 'custom',
    duration_days: parseInt(normalizeDigits(val)) || 90,
    current_stage: 0,
    review_count: parseInt(normalizeDigits(document.getElementById('srsReviewCount').value)) || 7,
    tags: document.getElementById('srsTags').value
  };
...
function changeSrsReviewCount(val){
  const kind = document.getElementById('srsDurationKind').value;
  const customDaysEl = document.getElementById('srsCustomDays');
  const draft = {
    id: editingSrsId,
    title: document.getElementById('srsTitle').value,
    start_date: document.getElementById('srsStartDate').value,
    duration_kind: kind,
    duration_days: kind==='custom' ? (parseInt(normalizeDigits(customDaysEl ? customDaysEl.value : '90')) || 90) : srsTotalDays(kind),
    current_stage: 0,
    review_count: parseInt(normalizeDigits(val)) || 7,
    tags: document.getElementById('srsTags').value
  };
  openSrsForm(draft);
}
...
async function saveSrsUnit(){
  ...
  const title = document.getElementById('srsTitle').value.trim();
  if(!title){ alert('اكتب اسم الوحدة التعليمية أولاً'); return; }
  const startDate = document.getElementById('srsStartDate').value;
  if(!startDate){ alert('حدد تاريخ بداية الوحدة'); return; }
  const kind = document.getElementById('srsDurationKind').value;
  const customDaysEl = document.getElementById('srsCustomDays');
  const durationDays = kind === 'custom'
    ? (parseInt(normalizeDigits(customDaysEl ? customDaysEl.value : '90')) || 90)
    : srsTotalDays(kind);
  const stage = parseInt(document.getElementById('srsCurrentStage').value) || 0;
  const reviewCount = parseInt(normalizeDigits(document.getElementById('srsReviewCount').value)) || 7;
  const payload = {
    title,
    ...
    tags: document.getElementById('srsTags').value.trim()
  };
  let error;
  if(id){
...
async function markSrsReviewed(id, currentStage){
  const {error} = await sb.from('srs_units').update({current_stage: currentStage + 1}).eq('id', id);
  if(error){ alert('صار خطأ: '+error.message); return; }
  await loadSrsUnits();
}
...
async function loadSrsUnits(){
  ...
  const dueCount = infos.filter(x => !x.info.done && x.info.diffDays <= 0).length;
  if(banner){
    banner.innerHTML = dueCount > 0
      ? `<div class="monthly-summary">🔔 عندك <b>${dueCount}</b> وحدة تحتاج مراجعة اليوم أو متأخرة.</div>`
      : '';
  }

  list.innerHTML = infos.map(({unit, info})=>{
    ...
    if(info.done){
      ...
      cardClass = info.diffDays <= 0 ? 'status-todo' : 'status-in_progress';
    ...
    <div class="path-card ${cardClass}">
      ...
      <div class="sj-name">${escapeHtml(unit.title)}</div>
      ${statusHtml}
      ...
      <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">
        ${(!info.done && info.diffDays <= 0) ? `<button class="primary" onclick="markSrsReviewed('${unit.id}', ${unit.current_stage||0})">تمت المراجعة ✓</button>` : ''}
        ${(!info.done && info.diffDays > 0) ? `<span style="font-size:.78rem;color:var(--ink-soft);align-self:center">🔒 تُفتح بعد ${info.diffDays} يوم</span>` : ''}
        <button class="ghost" data-srs="${dataAttr}" onclick="openSrsForm(JSON.parse(this.getAttribute('data-srs')))">تعديل</button>
        <button class="ghost" onclick="toggleSrsExpand('${unit.id}')">${expandedSrsIds.has(unit.id) ? 'إخفاء كل المواعيد ▴' : 'عرض كل المواعيد ▾'}</button>
      </div>
